...
var crearHorario = ()=>{
	...
	if( $("#modalidad-horario").text() != "" && $("#etapa-horario").val() != "" && $("#edad-minima").val() != "" && $("#edad-maxima").val() != "" && $("#vacantes-horario").val() != "" && $("#preinscripciones-horario").val() != "" &&  Object.values(totalTurnosAcumulados).length!=0 ){
		...
		if( ( parseInt($("#edad-minima").val()) <4 || parseInt($("#edad-maxima").val()) >17 ) && $("#modalidad-horario").val() == 0 && parseInt($("#idDisciplinaNewHorario").val()) == 21 ){

			alertify.error('Verifique la Edad Minima y Maxima para la Modalidad Convencional esten en el rango establecido.',4);

		}
		else if(( parseInt($("#edad-minima").val()) < 6 || parseInt($("#edad-maxima").val()) > 17 ) && $("#modalidad-horario").val() == 0 && parseInt($("#idDisciplinaNewHorario").val()) != 21){
			alertify.error('Verifique la Edad Minima y Maxima para la Modalidad Convencional esten en el rango establecido.',4);
		}

		else if( ( parseInt($("#edad-minima").val()) < 6 || parseInt($("#edad-maxima").val()) > 59 ) && $("#modalidad-horario").val() == 1){
			alertify.error('Verifique la Edad Minima y Maxima para la Modalidad Personas Discapacitadas esten en el rango establecido.',5);
