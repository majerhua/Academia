...
class ExportacionDataController extends Controller
{
  ...
  public function exportDataBeneficiariosAdminMacroAction(Request $request)
  {
  ...
  public function exportDataBeneficiariosMonitorAction(Request $request)
  {
    ...
    $response = new StreamedResponse(function() use($conn,$numMes,$departamento,$provincia,$idTemporada) {
      
      $query2='';
      
      if(!empty($numMes) && !empty($provincia) )
        $query2 = " AND ubiProv.ubidpto = '$departamento' AND ubiProv.ubiprovincia = '$provincia' AND MONTH(mov.fecha_modificacion) <= '$numMes' ";

      else if(!empty($numMes) && empty($provincia) )
        $query2 = "AND MONTH(mov.fecha_modificacion) <= '$numMes' "; 
        
      else if(empty($numMes) && !empty($provincia)  )
        $query2 = " AND ubiProv.ubidpto = '$departamento' AND ubiProv.ubiprovincia = '$provincia' ";     
      ...
      $queryConMes = "SELECT  ubiDpto.ubinombre Departamento,
                            ...
                            CONVERT(VARCHAR(100),STUFF(( SELECT  ',' +
                               ...
                               ubiProv.ubidpto <> '00' 
                               AND ubiProv.ubiprovincia <> '00'".$query2;
 ...
 public function exportDataBeneficiariosPromotorAction(Request $request)
  {
    $numMes = $request->query->get('mes');
    $idTemporada = $request->query->get('idTemporada');
    $idComplejo = $request->query->get('idComplejo');
 ...
 public function exportDataBeneficiariosProfesorAction(Request $request)
  {
    $numMes = $request->query->get('mes');
    $idTemporada = $request->query->get('idTemporada');
    $idComplejo = $request->query->get('idComplejo');
    $idDisciplina = $request->query->get('idDisciplina');

    $conn = $this->get('database_connection');

    $response = new StreamedResponse(function() use($conn,$numMes,$idComplejo,$idDisciplina,$idTemporada) {
                            ...
                            cat.descripcion Categoria,
                             ...
                             ,
                              CONVERT(VARCHAR(100),STUFF(( SELECT  ',' +
                               ...
                               ubiDpto.ubidpto <> '00' AND ede.ede_codigo='$idComplejo'
                               AND dis.dis_codigo = '$idDisciplina'; ";
  ...
  public function exportDataAction(Request $request){
    ...
    $mdlProvinciasExport = NULL;
    $mdlDepartamentosExport= NULL;
    $mdlComplejosExport = NULL;
    $mdlDisciplinasExport = NULL;
    ...
    if( $usuario->getIdPerfil() == $confPerfilUsuario['administrador'] ){
      ...
      $mdlDepartamentosExport = $em->getRepository('AkademiaBundle:Departamento')->departamentosExportByUsuarioId( $usuario->getId() );   
      
    }else if( $usuario->getIdPerfil() == $confPerfilUsuario['monitor'] ){
      $mdlProvinciasExport = $em->getRepository('AkademiaBundle:Departamento')->provinciasExportByUsuarioId( $usuario->getId() ); 

    }else if( $usuario->getIdPerfil() == $confPerfilUsuario['promotor'] ) {
      $mdlComplejosExport = $em->getRepository('AkademiaBundle:ComplejoDeportivo')->getComplejosUsuario($usuario->getId());

    }else if( $usuario->getIdPerfil() == $confPerfilUsuario['profesor'] ) {
      $mdlComplejosExport = $em->getRepository('AkademiaBundle:ComplejoDeportivo')->getComplejosDisciplinaUsuario($usuario->getId());
      $mdlDisciplinasExport = $em->getRepository('AkademiaBundle:ComplejoDeportivo')->getDisciplinaComplejoUsuario($usuario->getId());
    }
    ...
    return $this->render('AkademiaBundle:Export:export.html.twig',
                          array(  'departamentosExport' => $mdlDepartamentosExport,
                                  'provinciasExport' => $mdlProvinciasExport,
                                  'departamentosAll' => $mdlDepartamentos,
                                  'complejosExport' => $mdlComplejosExport,
                                  'disciplinasExport' => $mdlDisciplinasExport,
                                  'idTemporada' => $idTemporada,
                                  'descripcionTemporada' => $descripcionTemporada ,
                                  'mesInicioFinTemporada' => $mesesInicioAndFinTemporada[0] )); 
